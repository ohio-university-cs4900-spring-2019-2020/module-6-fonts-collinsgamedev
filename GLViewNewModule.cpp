...
std::string arial(ManagerEnvironmentConfiguration::getSMM() + "/fonts/arial.ttf");
...
void GLViewNewModule::onKeyDown( const SDL_KeyboardEvent& key )
{
   ...
   else if (key.keysym.sym == SDLK_BACKSPACE)
   {
	   ...
	   std::cout << " message erased " << msg << " " << std::endl;
   }
   else  //BUG too many backspaces causes crash
   {
	   char letter = (char)key.keysym.sym;
	   GLViewNewModule::msg.append(1, letter);
...
void Aftr::GLViewNewModule::loadMap()
{
   ...
   label->setFontPath(arial);
   worldLst->push_back(label);

   WOFTGLString* string = WOFTGLString::New(arial, 32);
   string->getModelT<MGLFTGLString>()->setFontColor(aftrColor4f(0.0f, 1.0f, 0.0f, 1.0f));
   string->getModelT<MGLFTGLString>()->setSize(32, 10);
   string->getModelT<MGLFTGLString>()->setText("Some other text");
   string->setText("Hello there");
